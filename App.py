...
connection = pymysql.connect(host='localhost',user='root',password='riddhi@2909',db='cv')
...
def run():
    ...
    if choice == 'User':
        ...
        if st.button('Login'):
            if ad_user == 'riddhi' and ad_password == 'riddhi123':
                st.success("Welcome to Resume AI !")
                # Display Data
                ...
                # Function to decode byte values to readable strings
                def decode_field(field):
                    try:
                        if isinstance(field, str):
                            # If field is a string representation of bytes (e.g., "68,97,116"), decode it
                            byte_values = bytes([int(x) for x in field.split(',')])
                            return byte_values.decode('utf-8')
                        elif isinstance(field, bytes):
                            # If it's already bytes, just decode it directly
                ...
                # Apply decoding function to relevant columns (Predicted Field and User Level)
                df['Predicted Field'] = df['Predicted Field'].apply(decode_field)
                df['User Level'] = df['User Level'].apply(decode_field)

                # ###### Pie chart for predicted field recommendations
                st.subheader("**Pie-Chart for Predicted Field Recommendation**")

                # Group by 'Predicted Field' and count occurrences
                ...
                fig1 = px.pie(predicted_field_counts, values='Count', names='Predicted Field', 
                ...
                # Group by 'User Level' and count occurrences
                user_level_counts = df['User Level'].value_counts().reset_index()
                user_level_counts.columns = ['User Level', 'Count']

                # Generate pie chart for User's Experienced Level
                fig2 = px.pie(user_level_counts, values='Count', names='User Level', 
